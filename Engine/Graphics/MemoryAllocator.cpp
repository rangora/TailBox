...
namespace tb
{
    ...
    void DescriptorPool::Initialize(int32 maxDescriptorCount)
    {
        ...
        _resourceDescriptorSize = g_renderAPI->GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

        D3D12_DESCRIPTOR_HEAP_DESC desc = {};
        desc.NumDescriptors = _maxDescriptorCount;
        desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
        desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
        if (S_OK != g_renderAPI->GetDevice()->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&_descriptorHeap)))
        {
    ...
    void ConstantBufferPool::Initialize(int32 blockSize, int32 maxSize)
    {
        ...
        if (S_OK != g_renderAPI->GetDevice()->CreateCommittedResource(&bufferProps, D3D12_HEAP_FLAG_NONE, &bufferDesc,
        ...
        if (S_OK != g_renderAPI->GetDevice()->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(&_descriptorHeap)))
        {
        ...
        {
            g_renderAPI->GetDevice()->CreateConstantBufferView(&cbvDesc, handle);
